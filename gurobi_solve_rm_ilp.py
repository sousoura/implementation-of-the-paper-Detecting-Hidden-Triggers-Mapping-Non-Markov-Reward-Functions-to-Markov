# gurobi_solve_reward_machine_ILP.py
...
def gurobi_solve_reward_machine_ILP(traces, K):
    ...
    model.Params.OutputFlag = 1  # 若不需要打印求解过程，可设置为 0
    ...
    # 1. 唯一性约束：对于每一步 (s,a,r,s_next)，必须唯一地分配一个 (u_i, u_j)
    for m, traj in enumerate(traces):
        for n in range(len(traj)):
            model.addConstr(
                gp.quicksum(O[(m, n, i, j)] for i in range(K) for j in range(K)) == 1,
                name=f"uniqueness_{m}_{n}"
            )

    # 2. 连续性约束：对于同一条轨迹，
    # 如果第 n 步分配为 (u_i -> u_j)，则第 n+1 步分配时其起始状态必须为 u_j
    for m, traj in enumerate(traces):
        for n in range(len(traj) - 1):
            for j in range(K):
                lhs = gp.quicksum(O[(m, n, i, j)] for i in range(K))
                rhs = gp.quicksum(O[(m, n + 1, j, k)] for k in range(K))
                model.addConstr(lhs == rhs, name=f"continuity_{m}_{n}_{j}")

    # 3. 确定性约束：
    # 对于全局中任意相同环境转移 (s,a,s_next)，若两步均分配到同一起始状态 u_i，
    # 则它们必须选取相同的目标状态 u_j。
    sa_sprime_steps = {}
    for m, traj in enumerate(traces):
        for n, (s, a, r, s_next) in enumerate(traj):
                    ...
                    for j in range(K):
                        for jp in range(K):
                            if j == jp:
                                continue
                            model.addConstr(
                                O[(m1, n1, i, j)] + O[(m2, n2, i, jp)] <= 1,
                                name=f"determinism_{key}_{i}_{m1}_{n1}_{m2}_{n2}_{j}_{jp}"
                            )

    # 4. 奖励一致性约束：
    # 对于全局中相同 (s,a,s_next) 的两步，如果它们的奖励不同，则它们不能被分配到相同的起始状态 u_i
    for key, steps in sa_sprime_steps.items():
        for idx1 in range(len(steps)):
            for idx2 in range(idx1 + 1, len(steps)):
                m1, n1, r1 = steps[idx1]
                ...
                if r1 != r2:
                    for i in range(K):
                        model.addConstr(
                            gp.quicksum(O[(m1, n1, i, j)] for j in range(K)) +
    ...
    # 6. 提取 ILP 求解结果：记录每条轨迹每一步对应的 (u_i, u_j)
    solution = {}
    for m, traj in enumerate(traces):
        solution[m] = []
        for n in range(len(traj)):
    ...
    # 7. 构造 Reward Machine 结构：
    # 对于每个步，根据 (u_i, s, a, s_next) 分组，得到对应的下一个状态 u_j 及奖励 r，
    # 即 δᵤ(u_i, (s,a,s_next)) = u_j, δᵣ(u_i, (s,a,s_next)) = r.
    rm_transitions = {}
    for m, traj in enumerate(traces):
    ...
    reward_machine = {
        "states": list(range(K)),
        "initial_state": 0,
        "transitions": {}  # key: (u_i, s, a, s_next) -> { "next_state": u_j, "reward": r, "steps": [...] }
    }
